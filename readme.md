...
* `TeammateIdle` hook: 종료 전 smoke test + stub 잔존 여부 확인 강제
...
* `maxTurns: 40`
* `TaskCompleted` hook: 종료 전 `evaluation-report.md` 갱신 여부 확인 강제
...
reviewer는 구현 결과를 품질 관점에서 비평하고 개선 방향을 제안하는 역할을 맡는다. reviewer는 pass/fail 심판이 아니라 개선 제안자다.

#### subagent 설정

* `model: opus` — 깊이 있는 품질 비평에 고성능 모델 사용
* `memory: project` — 누적 품질 지적, 개선 포인트 이력 관리
* `tools: Read, Glob, Grep` — 읽기 전용, 코드 수정 불가
* `maxTurns: 40`
* `TaskCompleted` hook: 종료 전 `review-notes.md` 갱신 여부 확인 강제
...
* 불필요한 복잡성 식별
* 기술 부채와 개선 포인트 제안
* 개선 우선순위 제시 (severity: critical / major / minor)

#### 산출물

* `review-notes.md`
* improvement recommendations (severity 구분)
* risk commentary
...
### 4.6 optional specialist reviewers

필요에 따라 reviewer 아래에 전문 리뷰어를 둘 수 있다.
...
1차 템플릿에서는 reviewer 하나로 시작하고, 내부 체크리스트 또는 후속 분리로 확장한다.
...
* TaskCompleted hook 스크립트 — evaluator/reviewer 산출물 파일 갱신 확인
...
| `review-notes.md` 작성 완료 | 사용자에게 다음 sprint 진행 여부 확인 |

### 14.2 사용자 승인이 필요한 전환

자동 전환 중 다음 두 시점에서는 반드시 멈추고 사용자 확인을 받는다.

* **planner 완료 후**: sprint-contract 초안을 사용자에게 제시하고 범위 승인을 받는다. 승인 없이 sprint-builder를 시작하지 않는다.
* **리뷰 완료 후**: 다음 sprint 범위를 제안하고 진행 여부를 확인한다. 자동으로 다음 sprint를 시작하지 않는다.
...
8. `claude-progress.txt`에 현재 상태와 다음 액션을 남긴다.
9. blocker가 있으면 integration-fixer 또는 다음 수정 sprint로 진행한다.
10. 사용자에게 다음 sprint 진행 여부를 확인한다.
