...
## OpenCode 네이티브 에이전트 시스템 (`.opencode/`)

OpenCode의 네이티브 primary/subagent 모드를 활용한 자동 오케스트레이션.
...
| `.opencode/agents/harness.md` | PRIMARY 오케스트레이터 (유일한 `mode: primary`) |
| `.opencode/agents/*.md` | 서브에이전트 17개 (`mode: subagent`) |
| `.opencode/rules/AGENTS.md` | 코딩 원칙 + 하네스 오케스트레이션 가이드 |

### 동작 원리

OpenCode 실행 시 `harness`가 유일한 primary 에이전트로 자동 선택된다.
harness는 `.claude-state/` 상태 파일을 읽고 `permission.task` 체인에 따라
Task 도구로 서브에이전트를 자동 호출한다.
...
harness (PRIMARY)
├── requirement-writer
├── planner → prd/userflow/dataflow/usecase-writer, stack-selector
├── sprint-builder → common-module/plan-writer, implementer, code-reviewer
├── evaluator
├── integration-fixer
├── reviewer
├── retrospective
